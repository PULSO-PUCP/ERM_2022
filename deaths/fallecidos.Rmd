...
poblacion <- read_excel("C:/Users/soyma/Documents/GitHub/ERM_2022/deaths/Matriz_2020.xlsx", sheet = 3,
                        skip = 8)
...
```{r}
fallecidosDist=aggregate(FECHA_CORTE~UBIGEO,data=fallecidos,FUN=length)
#UUID
```

```{r}
fallecidosDist$ubigeo_prov = substr(fallecidosDist$UBIGEO, 1, 4) 
```

```{r}
fallecidosProv=aggregate(FECHA_CORTE~ubigeo_prov,data=fallecidosDist,FUN=sum)
#UUID
```
...
mapaFallecidos = baseProv +
  geom_sf(aes(fill= cut(Tasa, breaks = 5)), size = 0.005) + labs(fill = "Tasa")+ 
